...
public:
     ...
     */
    void printMessage(const AbstractDecoder &decoder, bool verbose) const;

    /* Print message "python dictionary style" to be easily evaluated
     * by python code. Print all available headers.
     */
    void printMessageAsDict(const AbstractDecoder &decoder) const;

    /**
     * Print message processing statistics
     * @param decoder
     */
    void printStatistics(const AbstractDecoder &decoder) const;
