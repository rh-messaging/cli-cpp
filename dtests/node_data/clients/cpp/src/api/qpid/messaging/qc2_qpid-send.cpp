...
struct Options : public qpid::Options
{
          ...
          groupInterleave(1)
    {
        addOptions()
            ...
            ("ttl", qpid::optValue(ttl, "msecs"), "Time-to-live for messages, in milliseconds (or -1 for forever)")
            ...
            ("log-msgs", qpid::optValue(logMsgs, "LOGFMT"), "message[s] reporting style (body|upstream|none)")
...
int main(int argc, char ** argv)
{
    ...
    try {
        if (opts.parse(argc, argv)) {
            ...
            if (opts.ttl) {
                if (opts.ttl == -1)
                  msg.setTtl(Duration(Duration::FOREVER));
                else
            ...
            while (contentGen->setContent(msg)) {
                ...
                if (opts.timestamp)
                    msg.getProperties()[TS] = int64_t(duration_since_epoch());
                if (opts.logMsgs == "body")
                    std::cout << msg.getContent() << std::endl;
                else if (opts.logMsgs == "upstream")
