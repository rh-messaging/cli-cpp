...
struct Options : public qpid::Options {
    ...
    Options() : help(false),
                init(true), transfer(true), check(true),
                size(256), durable(false), queues(2),
                base("qc2_txtest"), msgsPerTx(1), txCount(5), totalMsgCount(10),
                capacity(1000), url("localhost"), quiet(false)
    {
        addOptions()
            ...
            ("msg-size", qpid::optValue(size, "N"), "message size")
            ("msg-durable", qpid::optValue(durable, "yes|no"), "use durable messages")
            ("queue-cnt", qpid::optValue(queues, "N"), "number of queues")
            ("queue-base-name", qpid::optValue(base, "<name>"), "base name for queues")
            ("tx-batch-size", qpid::optValue(msgsPerTx, "N"), "number of messages transferred per transaction")
            ("tx-cnt", qpid::optValue(txCount, "N"), "number of transactions per 'agent'")
            ("msg-cnt", qpid::optValue(totalMsgCount, "N"), "total number of messages in 'circulation'")
