...
void printUsage() {
  cout <<
"connector [(-b|--broker) <broker-url>] [(-c|--connections) <connection_amount>] \n\
...
/*
void hold_mode_exit(int signum) {
  list<qpid::messaging::Connection>::iterator con_list_it;
...
// future reserved
  ...
  for (int i = 1; i < argc; i++) {
    ...
    } else if (arg.compare("-c") == 0 || arg.compare("--conn-cnt") == 0) {
      if (++i<argc) {
...
*/
  ...
  if (obj_ctrl.find_first_of("CESR") != string::npos) {
    ...
    // ------------------------------------------------------------------------
    cout << opened << " " << not_opened << " " << conn_cnt << endl;
